...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WETH } from '@kwikswap/sdk'
...
import { useVExchangeContract, useV1MigratorContract } from '../../hooks/useContract'
...
export function VLiquidityInfo({
  token,
...
function VPairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  const { account, chainId } = useActiveWeb3React()
  const totalSupply = useTotalSupply(liquidityTokenAmount.token)
  const exchangeETHBalance = useETHBalances([liquidityTokenAmount.token.address])?.[liquidityTokenAmount.token.address]
  const exchangeTokenBalance = useTokenBalance(liquidityTokenAmount.token.address, token)

  const [v1PairState, v1Pair] = usePair(chainId ? WETH[chainId] : undefined, token)
  const isFirstLiquidityProvider: boolean = v1PairState === PairState.NOT_EXISTS

  const v1SpotPrice = chainId && v1Pair ? v1Pair.reserveOf(token).divide(v1Pair.reserveOf(WETH[chainId])) : undefined
  ...
  const minAmountToken: JSBI | undefined =
    v1SpotPrice && ethWorth
      ? ethWorth
          .multiply(v1SpotPrice)
    ...
    migrator
      ...
      .then((response: TransactionResponse) => {
        ...
        addTransaction(response, {
          summary: `Migrate ${token.symbol} liquidity to V1`
        })
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        This tool will safely migrate your V liquidity to V1 with minimal price risk. The process is completely
        trustless thanks to the{' '}
        {chainId && (
          <ExternalLink href={getEtherscanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">Kwikswap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into Kwikswap V1 at a price you believe is correct. If the V1 price seems
            incorrect, you can either make a swap to move the price or wait for someone else to do so.
          </TYPE.body>
          <AutoColumn gap="8px">
            <RowBetween>
              <TYPE.body>V Price:</TYPE.body>
              <TYPE.black>
                {vSpotPrice?.toSignificant(6)} {token.symbol}/ETH
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {vSpotPrice?.invert()?.toSignificant(6)} ETH/{token.symbol}
              </TYPE.black>
            </RowBetween>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on Kwikswap V1. Your liquidity will be migrated at the
            current V price. Your transaction cost also includes the gas to create the pool.
          </TYPE.body>
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your Kwikswap V ${token.symbol}/ETH liquidity will become Kwikswap V1 ${token.symbol}/ETH liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}

export default function MigrateVExchange({
  history,
  match: {
    params: { address }
  }
  ...
  const exchangeContract = useVExchangeContract(validatedAddress ? validatedAddress : undefined)
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `KWIK-${token.symbol}`, 'Kwikswap')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        {!account ? (
          ...
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because Kwikswap V1 uses WETH under the hood, your Kwikswap V WETH/ETH liquidity cannot be migrated. You
              may want to remove your liquidity instead.
            </TYPE.body>

            <ButtonConfirmed
              onClick={() => {
                history.push(`/remove/v/${validatedAddress}`)
              }}
            >
              Remove
            </ButtonConfirmed>
          ...
          <VPairMigration liquidityTokenAmount={userLiquidityBalance} token={token} />
        ) : (
